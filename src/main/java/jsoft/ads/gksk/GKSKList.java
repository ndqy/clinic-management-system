...
/**
 * Servlet implementation class GKSKList
 */
@WebServlet("/gksk/list")
public class GKSKList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (user != null) {
			//Xóa
			int delid = jsoft.library.Utilities.getIntParam(request, "delid");
			if (delid>0) {
				...
				item.setId(delid);					
				boolean result = uc.delGKSK(item);
				// Trả về kết nối
...
//					}
					...
					response.sendRedirect("/btl/gksk/list?err=urdel");
				}
			}else {
	...
	private void view(HttpServletRequest request, HttpServletResponse response, UserObject user)
		...
		Triplet<GKSKObject, Short, Byte> infors = new Triplet<>(similar, page, (byte) 100);

		List<String> viewList = gc.viewGKSK(infors);
		...
		out.append("<a href=\"/btl/gksk/add\" type=\"button\" class=\"btn btn-primary btn-sm my-3\">");
		out.append("<i class=\"bi bi-file-earmark-plus\"></i> Thêm mới");
		out.append("</a>");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		//Cập nhật
		int id = jsoft.library.Utilities.getIntParam(request, "idGKSK");
		if(user.getUser_id()>0 && user.getUser_permission()>1) {
			if (id>0) {
				...
				int benh_nhan_id = jsoft.library.Utilities.getIntParam(request, "idCustomer");
				...
				boolean result = uc.editGKSK(item);
